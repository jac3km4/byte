...
fn test_str() {
    ...
    let mut offset = 0;
    assert_eq!(
        bytes.read::<&str>(&mut offset, Delimiter(NULL)).unwrap(),
        "abcd"
    );
    assert_eq!(offset, 5);
    ...
    assert_eq!(
        TryRead::try_read(bytes, Len(15)).unwrap(),
        ("abcdefghijklmno", 15)
    );
    assert_eq!(
        TryRead::try_read(bytes, Len(26)).unwrap(),
        ("abcdefghijklmnopqrstuvwxyz", 26)
    );

    assert!(bytes.read::<&str>(&mut 0, Len(27)).is_err());
    assert!(bytes.read::<&str>(&mut 27, Len(0)).is_err());
    assert!(bytes.read::<&str>(&mut 26, Len(1)).is_err());
}
...
fn test_str_delimitor() {
    let bytes: &[u8] = b"";
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 0)).unwrap(),
        ("", 0)
    );

    let bytes: &[u8] = b"abcdefg";
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 6)).unwrap(),
        ("abcdef", 6)
    );
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 7)).unwrap(),
        ("abcdefg", 7)
    );

    let bytes: &[u8] = b"\0abcdefg";
    assert_eq!(TryRead::try_read(bytes, Delimiter(NULL)).unwrap(), ("", 1));
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 0)).unwrap(),
        ("", 0)
    );
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 1)).unwrap(),
        ("", 1)
    );

    let bytes: &[u8] = b"abcd\0efg";
    assert_eq!(
        TryRead::try_read(bytes, Delimiter(NULL)).unwrap(),
        ("abcd", 5)
    );
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 4)).unwrap(),
        ("abcd", 4)
    );
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 5)).unwrap(),
        ("abcd", 5)
    );
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 6)).unwrap(),
    ...
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 8)).unwrap(),
        ("abcdefg", 8)
    );
    assert_eq!(
        TryRead::try_read(bytes, DelimiterUntil(NULL, 20)).unwrap(),
        ("abcdefg", 8)
    );

    let bytes: &[u8] = b"";
    assert!(bytes.read::<&str>(&mut 0, Delimiter(NULL)).is_err());
    assert!(bytes.read::<&str>(&mut 0, DelimiterUntil(NULL, 1)).is_err());

    let bytes: &[u8] = b"abcdefg";
    assert!(bytes.read::<&str>(&mut 0, DelimiterUntil(NULL, 8)).is_err());
    assert!(bytes.read::<&str>(&mut 0, Delimiter(NULL)).is_err());
}
...
fn test_bytes() {
    ...
    assert_eq!(TryRead::try_read(&bytes, Len(4)).unwrap(), (&bytes[..], 4));

    assert!(bytes.read::<&[u8]>(&mut 5, Len(0)).is_err());
...
fn test_bytes_pattern() {
    ...
    assert_eq!(
        TryRead::try_read(bytes, Pattern(b"abc")).unwrap(),
        (&b"abc"[..], 3)
    );
    assert_eq!(
        TryRead::try_read(bytes, Pattern(b"cde")).unwrap(),
        (&b"abcde"[..], 5)
    );
    assert_eq!(
        TryRead::try_read(bytes, Pattern(b"jk")).unwrap(),
        (&b"abcdefghijk"[..], 11)
    );
    assert_eq!(
        TryRead::try_read(bytes, PatternUntil(b"abc", 3)).unwrap(),
        (&b"abc"[..], 3)
    );
    assert_eq!(
        TryRead::try_read(bytes, PatternUntil(b"abc", 4)).unwrap(),
        (&b"abc"[..], 3)
    );
    assert_eq!(
        TryRead::try_read(bytes, PatternUntil(b"cde", 3)).unwrap(),
        (&b"abc"[..], 3)
    );
    assert_eq!(
        TryRead::try_read(bytes, PatternUntil(b"cde", 4)).unwrap(),
        (&b"abcd"[..], 4)
    );
    assert_eq!(
        TryRead::try_read(bytes, PatternUntil(b"cde", 5)).unwrap(),
        (&b"abcde"[..], 5)
    );
    assert_eq!(
        TryRead::try_read(bytes, PatternUntil(b"cde", 6)).unwrap(),
        (&b"abcde"[..], 5)
    );
    assert_eq!(
        TryRead::try_read(bytes, PatternUntil(b"xyz", 5)).unwrap(),
        (&b"abcde"[..], 5)
    );
    assert!(bytes.read::<&[u8]>(&mut 0, Pattern(b"xyz")).is_err());
    assert!(bytes.read::<&[u8]>(&mut 0, Pattern(b"")).is_err());
    assert!(bytes.read::<&[u8]>(&mut 0, PatternUntil(b"", 3)).is_err());
    assert!(bytes
        .read::<&[u8]>(&mut 0, PatternUntil(b"abcd", 3))
        .is_err());
    assert!(bytes
        .read::<&[u8]>(&mut 0, PatternUntil(b"xyz", 20))
        .is_err());

    let bytes: &[u8] = b"";
    assert!(bytes.read::<&[u8]>(&mut 0, Pattern(b"xyz")).is_err());
    assert!(bytes
        .read::<&[u8]>(&mut 0, PatternUntil(b"abc", 3))
        .is_err());
    assert!(bytes
        .read::<&[u8]>(&mut 0, PatternUntil(b"abc", 4))
        .is_err());
}
...
macro_rules! test_num {
    ($test_name: tt, $ty: ty, $byteorder_read_fn: tt, $byteorder_write_fn: tt) => {
        quickcheck! {
            fn $test_name (num: $ty) -> () {
                let mut bytes = [0u8; 8];
                bytes.write(&mut 0, &num, LE).unwrap();
                let result = byteorder::LittleEndian::$byteorder_read_fn(&bytes);
                assert_eq!(result, num);

                let mut bytes = [0u8; 8];
                byteorder::LittleEndian::$byteorder_write_fn(&mut bytes, num);
                let result: $ty = bytes.read(&mut 0, LE).unwrap();
                assert_eq!(result, num);

                let mut bytes = [0u8; 8];
                bytes.write(&mut 0, &num, BE).unwrap();
                let result = byteorder::BigEndian::$byteorder_read_fn(&bytes);
                assert_eq!(result, num);

                let mut bytes = [0u8; 8];
                byteorder::BigEndian::$byteorder_write_fn(&mut bytes, num);
                let result: $ty = bytes.read(&mut 0, BE).unwrap();
                assert_eq!(result, num);
            }
        }
    };
